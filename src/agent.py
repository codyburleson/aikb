...
async def main():
    ...
    folder_context = ", ".join(existing_folders)
    print(f"📝 Context Loaded: Templates=[{available_templates}] | Folders=[{folder_context}]")
    ...
    2. EXISTING FOLDERS: [{folder_context}]
    ...
    root_agent = LlmAgent(
        ...
        name='AIKB_Agent',
        # app_name removed to prevent Pydantic validation error
    ...
    await session_service.create_session(app_name="aikb_local", user_id=USER_ID, session_id=SESSION_ID)
